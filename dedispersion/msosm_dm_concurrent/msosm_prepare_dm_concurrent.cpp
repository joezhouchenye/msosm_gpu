...
Prepare_MSOSM_DM_concurrent::Prepare_MSOSM_DM_concurrent(float *bw, float *dm, float *f0, int numDMs) : bw(bw), dm(dm), f0(f0), numDMs(numDMs)
{
    fs = bw;
    kdm = 4.15 * 1e15;
    w0 = new float[numDMs];
    ...
    delaycount = new int[numDMs];
    dedisp_params = new fftwf_complex *[numDMs];
    delay_points = new int *[numDMs];
}

void Prepare_MSOSM_DM_concurrent::calculate_min_order()
{
...
void Prepare_MSOSM_DM_concurrent::override_order(unsigned long order)
{
    M_common = order;
}

void Prepare_MSOSM_DM_concurrent::generate_dedisp_params(int i)
{
...
void Prepare_MSOSM_DM_concurrent::segmentation(int i)
{
    delaycount[i] = static_cast<int>(ceil(Nd[i] / static_cast<float>(M_common)));
    if (verbose)
    {
    ...
    {
        w[j] = (sqrt(1 / (1 / pow(w0[i], 2) - j * M_common / fs[i] / (4 * pi * pi * kdm * dm[i]))) - w0[i] - pi * fs[i]) / pi / fs[i];
        if (w[j] < 0)
            pos_index = j + 1;
    }
    negcount = pos_index;
    ...
    {
        segneg[j] = static_cast<unsigned long>(floor((w[j] + 2) * M_common));
    }
    segneg[negcount] = 2 * M_common;
    segpos[0] = 0;
    for (unsigned long j = 0; j < poscount; j++)
    {
        segpos[j + 1] = static_cast<unsigned long>(floor(w[j + negcount] * M_common));
    }
    ...
    delay_points[i] = (int *)malloc(sizeof(int) * 2 * M_common);
    for (unsigned long j = 0; j < negcount; j++)
    {
