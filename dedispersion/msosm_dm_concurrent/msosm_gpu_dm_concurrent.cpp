...
void MSOSM_GPU_DM_concurrent::initialize_uint16(unsigned long fftpoint, int count)
{
    ...
    CUDA_CHECK(cudaMalloc((void **)&input_buffer_int16_d, count * M_common * sizeof(uint16_pair)));
...
void MSOSM_GPU_DM_concurrent::filter_block_uint16(uint16_pair *input)
{
    ...
    CUDA_CHECK(cudaMemcpyAsync(input_buffer_int16_d, input, count * M_common * sizeof(uint16_pair), cudaMemcpyHostToDevice, fft_stream));
    // 拷贝重合部分到输入缓冲区
    ...
    // 转换为复数float
    uint16ToComplex(input_buffer_d + M_common, (uint16_t *)(input_buffer_int16_d), count * M_common, fft_stream);
...
{
    complexToUint16((uint16_t *)output_buffer_uint16_d, output_buffer_d, count * M_common * numDMs);
    while (wait_for_cpu)
        ;
    wait_for_cpu = true;
    cudaStreamWaitEvent(output_stream, ready_event, 0);
